...
public class TaskFlowModel
{
    ...
    {
        get => string.IsNullOrEmpty(Requirements) ? new List<string>() : Requirements.Split(',').Select(r => r.Trim()).ToList();
        set => Requirements = value != null ? string.Join(",", value) : "";
    }

    [NotMapped]
    public List<string> CheckedRequirements
    {
        get => string.IsNullOrEmpty(DoneRequirements) ? new List<string>() : DoneRequirements.Split(',').Select(r => r.Trim()).ToList();
        set => DoneRequirements = value != null ? string.Join(",", value) : "";
    }
}
